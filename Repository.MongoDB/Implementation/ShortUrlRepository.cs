...
namespace Repository.MongoDB.Implementation
{
    public class ShortUrlRepository : IShortUrlRepository
    {
        ...
        public async Task<string> ExistAsync(string longUrl)
        {
            var filter = Builders<ShortUrl>.Filter.Eq(x => x.LongUrl, longUrl);
            var projection = Builders<ShortUrl>.Projection.Expression(v => v.Id);
            var id = await _collection.Find(filter).Project(projection).FirstOrDefaultAsync();
            return id;
        }
        ...
        {
            var filter = Builders<ShortUrl>.Filter.Eq(x => x.Id, id);
            var update = Builders<ShortUrl>.Update.Inc(x => x.RedirectionsCount, 1).Set(x => x.LastRedirectDate, DateTime.UtcNow);
